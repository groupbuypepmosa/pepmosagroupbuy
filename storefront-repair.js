...
(function(){
  ...
  function styles(){if($('pepStableStoreStyles'))return;const s=document.createElement('style');s.id='pepStableStoreStyles';s.textContent=`
    ...
    .pepKitCompletionCard{position:relative;overflow:hidden;margin:18px 0 22px;padding:26px 28px;border:1px solid rgba(213,43,135,.22);border-radius:26px;background:linear-gradient(135deg,rgba(255,255,255,.98),rgba(255,237,247,.94) 55%,rgba(248,239,255,.96));box-shadow:0 18px 45px rgba(178,58,130,.10)}
    .pepKitGlow{position:absolute;width:230px;height:230px;border-radius:50%;right:-90px;top:-120px;background:radial-gradient(circle,rgba(235,67,154,.16),transparent 70%);pointer-events:none}
    .pepKitTop{position:relative;display:flex;align-items:center;justify-content:space-between;gap:16px}
    .pepKitBadge{display:inline-flex;align-items:center;gap:8px;padding:10px 14px;border-radius:999px;background:linear-gradient(135deg,#e52b8b,#b93bb6);color:#fff;font-size:12px;font-weight:950;letter-spacing:.08em;box-shadow:0 10px 24px rgba(205,43,139,.22)}
    .pepKitIcon{width:50px;height:50px;display:grid;place-items:center;border-radius:17px;background:#fff;color:#cf3387;font-size:25px;box-shadow:0 10px 24px rgba(150,70,130,.10)}
    .pepKitCompletionCard h3{position:relative;margin:18px 0 7px;color:#512a40;font-size:clamp(28px,4vw,40px);line-height:1.04;letter-spacing:-.04em}
    .pepKitLead{position:relative;margin:0;color:#725c68;font-size:16px}
    .pepKitRules{position:relative;display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px;margin-top:20px}
    .pepKitRules>div{display:flex;gap:10px;align-items:flex-start;padding:14px;border:1px solid rgba(213,43,135,.12);border-radius:17px;background:rgba(255,255,255,.72)}
    .pepKitRules span{flex:0 0 28px;width:28px;height:28px;display:grid;place-items:center;border-radius:50%;background:#f7d8e8;color:#bd3479;font-size:12px;font-weight:950}
    .pepKitRules p{margin:2px 0 0;color:#6f5b65;font-size:13px;line-height:1.45}
    .pepKitFooter{position:relative;margin-top:15px;padding:13px 15px;border-radius:14px;background:rgba(255,255,255,.64);color:#7c5367;font-size:13px;font-weight:700}
    @media(max-width:760px){.pepKitCompletionCard{padding:22px 18px;border-radius:22px}.pepKitRules{grid-template-columns:1fr}.pepKitCompletionCard h3{font-size:30px}.pepKitIcon{width:44px;height:44px}}
  ...
  function ensureUI(){styles();const status=$('gbStatus');if(status&&!$('pepKitCompletionCard')){const k=document.createElement('section');k.id='pepKitCompletionCard';k.className='pepKitCompletionCard';k.style.cssText='display:none';k.innerHTML='<div class="pepKitGlow"></div><div class="pepKitTop"><div class="pepKitBadge">✦ KIT COMPLETION</div><div class="pepKitIcon">◈</div></div><h3>Complete the remaining vials</h3><p class="pepKitLead">Each <b>mg / variant is tracked separately.</b></p><div class="pepKitRules"><div><span>1</span><p>Every strength has its own separate 10-vial kit and live remaining count.</p></div><div><span>2</span><p>Only variants with remaining vials are shown.</p></div><div><span>3</span><p>You may order from <b>1 vial up to the exact live remaining quantity.</b></p></div><div class="pepKitFooter">⚡ Once a remaining vial is secured, it automatically becomes unavailable to the next buyer.</div>';const grid=$('productGrid');if(grid&&grid.parentNode)grid.parentNode.insertBefore(k,grid);else status.parentNode.insertBefore(k,status.nextSibling)}/* Group Buy Access controls remain available through the payment/verification flow, but are not displayed as a separate Home-page block. */
    makeModal('pepFeeModal',`<div class="pepFeeBox"><div class="pepFeeHead"><div class="pepFeeKicker">PEPMOSA GROUP BUY</div><h2>Group Buy Access ✦</h2></div><div class="pepFeeBody"><div id="pepFeeGb" class="pepFeeSummary"></div><div id="pepFeePaymentInfo" class="pepFeePaymentGrid"></div><div class="pepFeeField"><label>Email *</label><input id="pepFeeEmail" type="email" placeholder="your@email.com" autocomplete="email"></div><div class="pepFeeField"><label>Full Name *</label><input id="pepFeeName" placeholder="Your full name" autocomplete="name"></div><div class="pepFeeField"><label>WhatsApp Name *</label><input id="pepFeeWhatsApp" placeholder="Your WhatsApp name"></div><div class="pepFeeField"><label>Mobile Number *</label><input id="pepFeePhone" placeholder="09xxxxxxxxx" inputmode="tel"></div><div class="pepFeeField"><label>Payment Proof *</label><div class="pepFeeUpload"><input id="pepFeeProof" type="file" accept="image/*,.jpg,.jpeg,.png,.webp,.jfif,.pdf"><div class="pepFeeFileHint">Upload your receipt or screenshot. JPG, PNG, WEBP, JFIF or PDF • max 5MB.</div></div></div><div id="pepFeeMsg"></div><div class="pepFeeActions"><button id="pepFeeSubmit" class="pepFeeBtn">I HAVE PAID</button><button class="pepFeeBtn secondary" onclick="document.getElementById('pepFeeModal').classList.remove('open')">CANCEL</button></div></div></div>`);
  ...
  // Product photo treatment styles injected at runtime so the customer storefront always receives them.
  const __pepPinkStudioCSS=`
#productGrid .pepPinkStudio .productImg{background:radial-gradient(circle at 15% 18%,rgba(255,255,255,.9),transparent 32%),radial-gradient(circle at 86% 82%,rgba(255,206,226,.72),transparent 25%),linear-gradient(135deg,#f8c4d8,#f8dce6 55%,#edb6cb)!important;position:relative!important;overflow:hidden!important}
#productGrid .pepPinkStudio .productImg:before{content:"✿";position:absolute;left:8px;bottom:4px;font:52px Georgia;color:rgba(255,255,255,.38);z-index:0}
#productGrid .pepPinkStudio .productImg img{position:relative!important;z-index:1!important;width:112%!important;height:112%!important;max-width:none!important;max-height:none!important;object-fit:contain!important;object-position:center!important;transform:scale(1.10)!important;filter:drop-shadow(0 16px 16px rgba(110,45,75,.25)) saturate(1.05)!important}
#productGrid .pepKeepOriginal .productImg img{transform:none!important}
`;
