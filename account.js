...
function switchTab(which){const login=which==='login';$('loginTab').classList.toggle('active',login);$('signupTab').classList.toggle('active',!login);$('loginPanel').classList.toggle('active',login);$('signupPanel').classList.toggle('active',!login);msg('', '');show('message',false)}
async function signup(e){e.preventDefault();const name=$('signupName').value.trim(),email=$('signupEmail').value.trim(),p=$('signupPassword').value,p2=$('signupPassword2').value;if(p!==p2)return msg('Passwords do not match.','error');const {data,error}=await sb.auth.signUp({email,password:p,options:{data:{full_name:name},emailRedirectTo:location.origin+'/account.html'}});if(error)return msg(error.message,'error');if(data.session){await refresh();return}msg('Account created. Please verify your email first. After verification, your account will be reviewed by PEPMOSA admin before it becomes active.','success')}
