...
function renderEnhanced(){
 ...
 list.innerHTML=data.map(function(u){
   ...
   const controls='<button class="action primaryAction" type="button" data-customer-id="'+esc2(u.id)+'">VIEW CUSTOMER</button>'+(!verified&&status==='PENDING_EMAIL'?'<button class="action approve" type="button" onclick="verifyUser(\''+esc2(u.id)+'\',this)">VERIFY EMAIL</button>':'')+(status!=='APPROVED'&&verified?'<button class="action approve" type="button" onclick="approveUser(\''+esc2(u.id)+'\')">APPROVE</button>':'')+(status!=='REJECTED'?'<button class="action" type="button" onclick="rejectUser(\''+esc2(u.id)+'\')">REJECT</button>':'')+(status==='APPROVED'||status==='REJECTED'?'<button class="action delete-account" type="button" onclick="deleteUser(\''+esc2(u.id)+'\',this)">DELETE PERMANENTLY</button>':'');
   return '<div class="user"><div class="row"><div class="customerMain"><div class="email">'+esc2(u.full_name||'NO NAME')+'</div><div class="meta strong">Email: '+esc2(u.email||'—')+'</div><div class="accountStats"><span><b>CART</b>'+cq+' item'+(cq===1?'':'s')+'</span><span><b>ORDERS</b>'+s.orders.length+'</span><span><b>TOTAL SPENT</b>'+money(spent)+'</span><span><b>LOGINS</b>'+s.logins.length+'</span></div><div class="detailGrid"><div><b>Contact</b><span>'+esc2(u.contact_number||'—')+'</span></div><div><b>WhatsApp</b><span>'+esc2(u.whatsapp_name||'—')+'</span></div><div class="wide"><b>Complete Address</b><span>'+esc2(u.address||'—')+'</span></div></div><div class="meta">Created: '+date2(u.created_at)+' • Approved: '+date2(u.approved_at)+'</div><div class="meta">Email: <span class="badge '+(verified?'verified':'unverified')+'">'+(verified?'VERIFIED':'NOT VERIFIED')+'</span></div></div><span class="badge '+(status==='APPROVED'?'approved':status==='REJECTED'?'rejected':'pending')+'">'+esc2(status.replaceAll('_',' '))+'</span></div><div class="actions">'+controls+'</div></div>';
 }).join('');
...
window.verifyUser=async function(id,btn){if(!confirm('Manually verify this customer email? This bypasses the email OTP.'))return;if(btn){btn.disabled=true;btn.textContent='VERIFYING...'}const r=await sb2.functions.invoke('admin-verify-customer-email',{body:{user_id:id}});if(r.error||r.data?.error){alert(r.error?.message||r.data?.error||'Verification failed.');if(btn){btn.disabled=false;btn.textContent='VERIFY EMAIL'}return}loadEnhanced()};
window.approveUser=async function(id){const p=await sb2.from('profiles').select('email_verified_at').eq('id',id).maybeSingle();if(!p.data?.email_verified_at){alert('Verify the customer email first.');return}const r=await sb2.from('profiles').update({account_status:'APPROVED',approved_at:new Date().toISOString(),rejected_at:null}).eq('id',id);if(r.error)alert(r.error.message);else loadEnhanced()};
window.rejectUser=async function(id){if(!confirm('Reject this account?'))return;const r=await sb2.from('profiles').update({account_status:'REJECTED',rejected_at:new Date().toISOString(),approved_at:null}).eq('id',id);if(r.error)alert(r.error.message);else loadEnhanced()};
