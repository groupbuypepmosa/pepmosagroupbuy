...
`;document.head.appendChild(s);
  ...
  function insertUI(){
    ...
    if($('pepTrust'))$('pepTrust').remove();
    /* Bottom trust cards duplicate the Quick Access links, so keep Quick Access as the single navigation row. */
    if(false&&products&&!$('pepTrust')){
  ...
  async function loadMOQProducts(){
    ...
    try{
      ...
      try{
        ...
        if(roundRes.data){moqName=String(roundRes.data.moq_name||'').trim();round=String(roundRes.data.moq_number||'1').trim()||'1';globalTarget=Number(roundRes.data.moq_target||0)||0;}
